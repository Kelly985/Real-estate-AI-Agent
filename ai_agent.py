...
GREETING_RESPONSE = "Hello! How can I assist you with your real estate needs today?"
...
class AudioHandler:
    ...
    def text_to_speech(self, text):
        ...
        try:
            ...
            audio_file = f"tts_output_{uuid.uuid4()}.mp3"
            tts.save(audio_file)
            logger.info(f"gTTS generated audio file: {audio_file}")
            return audio_file
        except Exception as e:
            logger.error(f"gTTS speech synthesis failed: {str(e)}")
            return None
...
class AIAgent:
    ...
    def preprocess_query(self, query: str) -> str:
        ...
        # Normalize to lowercase and remove punctuation for greeting checks
        normalized_query = query.lower().strip(".,!?")
        logger.debug(f"Preprocessed query: {query} (normalized: {normalized_query})")
    ...
    def generate_response(self, query: str, mode: str = "text") -> str:
        ...
        try:
            ...
            if query in GREETING_KEYWORDS:
                ...
                self.conversation.add_entry(original_query, GREETING_RESPONSE, mode)
...
def main():
    ...
    # Test audio file processing (assuming a sample audio file exists)
    test_audio = "test_audio.wav"
    if os.path.exists(test_audio):
        try:
            print("\nTesting audio file transcription...")
            transcription = agent.audio.transcribe(test_audio)
            ...
            print("Testing TTS...")
            audio_file = agent.audio.text_to_speech(response)
            if audio_file:
                print(f"TTS successful, generated: {audio_file}")
            else:
                print("TTS failed")
        except Exception as e:
